...
class Blog extends CI_Controller {
	...
	public function index($onset = 0)
	{
		...
		$data['category_tablelist'] = $this->M_cloud->findAll('blog_category_mange', 'catId desc');
		...
		$data['product_tablelist'] = $this->M_cloud->tbOn5ObyResult('blog_mange_table', $onset, 'wproid desc');
		$data['onset'] 			= $onset;
		$config['base_url'] 	= base_url('websiteloginControler/blog_mange_table/index');
		$config['total_rows'] 	= $this->M_cloud->tbRowCount50('blog_mange_table', 'wproid asc');
		...
		$this->load->view('websiteloginview/Blog_managePage', $data);
	}
	...
	public function Show_now()
	{
		...
		$data['product_tablelist'] 		= $this->M_cloud->subcatproduct('blog_mange_table', $where, 'wproid asc');
		$this->load->view('websiteloginview/BlogShorting_product_ListPage', $data);
	}
	...
	public function store()
	{
		...
		if(!empty($id)){
			...
			$this->M_cloud->updateAll('blog_mange_table', $data, array('wproid' => $id));
			...
			$this->M_cloud->save('blog_mange_table', $data);
			$this->session->set_userdata(array('msg' => 'Data has been saved successfully.'));
		}
		redirect('websiteloginControler/Blog');
	}
	...
	public function update()
	{
		...
		$id = $this->input->post('id');
		$result = $this->M_cloud->find('blog_mange_table', array('wproid' => $id));
	...
	public function delete($wproid)
	{
		...
		$result 	= $this->M_cloud->tbWhRow('blog_mange_table', $where);
		$this->M_cloud->destroyAll('blog_mange_table', $where);
