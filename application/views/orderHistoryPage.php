  ...
  <div class="app catalog navOpen theme navBtn">
    ...
    <div class="landingPage2">
        <div class="order-history-page">
                            <link rel="stylesheet" href="css/print.css?v=20210210113926" media="all"/>
                <div class="order-list">
					...
					<?php }?>
                        ...
                        <tbody>
						<?php $sn = 1; foreach($orderuserinfo as $v){ ?>
							<tr>
							<td>
							...
							<?php } else if($language_check == 'BN') { ?>
							<?php $Sl =  $sn++;?>
							<?php  echo BanglaConverter::en2bn("$Sl\n"); ?>
							<?php } ?>
							</td>
							<td><a href="<?php echo site_url("customer_order_view/index/".$v->orId); ?>">
							
							<?php if(($language_check == 'EN') || (empty($language_check))) { ?>
							<?php echo $v->invoice_no; ?>
							<?php } else if($language_check == 'BN') { ?>
							<?php  echo BanglaConverter::en2bn("$v->invoice_no\n"); ?>
							<?php } ?>
							</a></td>
							<td>
							
							<?php if(($language_check == 'EN') || (empty($language_check))) { ?>
							<?php echo $newDate = date("d-m-Y", strtotime($v->paymentdate)); ?>
							<?php } else if($language_check == 'BN') { ?>
							<?php echo $newDate = date("d-m-Y", strtotime($v->paymentdate)); ?>
							<?php } ?>
							</td>
							...
							if($v->coupon_type == 'Percentage'){
							    $order_amount_per = ($v->total_amount*$v->coupon_amount)/100;
							    $Final_order_amount_per = ($v->total_amount-$order_amount_per);
							    $LastFinal_order_amount_per = ($Final_order_amount_per+$v->delevery_cost);
							    
							} else if($v->coupon_type == 'Fixed Amount'){
							    $LastFinal_order_amount_per = ($v->total_amount-$v->coupon_amount)+$v->delevery_cost;
							}else {
							    $LastFinal_order_amount_per = ($v->total_amount)+$v->delevery_cost;
							}
							...
							<?php if(($language_check == 'EN') || (empty($language_check))) { ?>
							<?php echo $LastFinal_order_amount_per; ?>
							<?php } else if($language_check == 'BN') { ?>
							<?php  echo BanglaConverter::en2bn("$LastFinal_order_amount_per\n"); ?>
							<?php } ?>
							</td>
                ...
                <script type="text/javascript">
                    ...
                    var submitForm = function (inputInvoiceNumber) {
                        $.post("submit/track_submit.php?inputInvoiceNumber=" + inputInvoiceNumber + "&page=orders", function (data) {
                    ...
                    var add_to_cart_again = function (salesid) {
                        alert(salesid)
                        $("#add_bag_txt_" + salesid).html('<center><img src="images/loader_small.gif" alt=""></center>');
                        $(".add_bag_txt_" + salesid).load('order_again.php?salesid=' + salesid, function () {
                            $("#add_bag_txt_" + salesid).html(' Order Again');
                        });
                    }
                </script>
                    </div>
