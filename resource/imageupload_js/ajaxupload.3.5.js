...
$(window).ready(function(){
	...
	$('.attachmentbody').on('click', '.remove', function(){
		...
		$.ajax({
			url: SAWEB.getSiteAction('upload/remove/'+el.attr("data-type")),
			type: "POST",
			dataType: "html",
			data: {attachment:$(el.attr("data-target")).val()},
			complete: function () {
				$(el.attr("data-target")).val("");
				el.html('<img class="upload" src="'+SAWEB.getBaseAction('resource/img/no_image.jpg')+'" />');
	...
	$.each($(document).find('.attachmentbody'), function(i, el) {
		...
		if(el.find('.upload').length > 0) {
			new AjaxUpload(el.find('.upload'), {
				action: SAWEB.getSiteAction('upload/index/'+el.attr("data-type")),
				name: 'attachment',
				responseType: 'json',
				...
				onComplete: function(file, response){                    
					...
					if(response.status === "success"){
						...
						$('<li></li>').appendTo(list).html('<img src="'+SAWEB.getBaseAction(response.fileLocation)+'" alt="" />');                            
